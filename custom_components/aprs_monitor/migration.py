...
def normalized_runtime_options(
    ...
    update_interval = (
        normalize_update_interval(options.get(CONF_UPDATE_INTERVAL))
        or DEFAULT_UPDATE_INTERVAL
    )
    max_position_age = (
        normalize_max_position_age(options.get(CONF_MAX_POSITION_AGE))
        or DEFAULT_MAX_POSITION_AGE
    )
    home_radius = (
        normalize_home_radius(options.get(CONF_HOME_RADIUS)) or DEFAULT_HOME_RADIUS
    )
    movement_threshold = (
        normalize_movement_speed_threshold(
            options.get(CONF_MOVEMENT_SPEED_THRESHOLD)
        )
